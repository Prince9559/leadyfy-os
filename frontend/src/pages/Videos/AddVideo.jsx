...
function AddVideo() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [clientsData, ordersData, scriptsData, usersData] = await Promise.all([
          getClients(),
          getOrders(),
          getScripts(),
          getUsers(),
        ...
        toast.error(error.response?.data?.message || "Failed to load video form data");
      } finally {
  ...
  return (
    <div className="videos-page">
      <div className="videos-header">
        ...
        <button type="button" className="video-back-button" onClick={() => navigate("/videos")}>
          <ArrowLeft size={18} />
          Back to Videos
        </button>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="video-form-grid">
            ...
            <div className="video-form-group">
              ...
              <select name="script" value={formData.script} onChange={handleChange} disabled={!formData.client || !formData.order}>
                <option value="">
                  {!formData.client ? "Select Client First" : !formData.order ? "Select Order First" : "Select Script (Optional)"}
                </option>
            ...
            <div className="video-form-group">
              ...
              <select name="assignedEditor" value={formData.assignedEditor} onChange={handleChange}>
          ...
          <div className="video-form-actions">
            <button type="button" className="video-cancel-button" onClick={() => navigate("/videos")} disabled={saving}>
              Cancel
            </button>

            <button type="submit" className="video-save-button" disabled={saving}>
              <Save size={18} />

...
import { getShootById, updateShoot } from "../../services/shootService";
...
function EditShoot() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [shootData, clientData, orderData, creatorData] = await Promise.all([
          getShootById(id),
          getClients(),
          getOrders(),
          getCreators(),
        ...
        const shoot = shootData.shoot || shootData.data;
        if (!shoot) {
        ...
        setFormData({
          ...
          order: shoot.order?._id || shoot.order || "",
          creator: shoot.creator?._id || shoot.creator || "",
        ...
        console.error("Load edit shoot data error:", error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate(`/shoots/view/${id}`);
      }, 800);
    } catch (error) {
      console.error("Update shoot error:", error);
      toast.error(error.response?.data?.message || "Failed to update shoot");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="shoots-page">
      <div className="shoots-header">
        <div>
          <h1>Edit Shoot</h1>
          <p>Update shoot information</p>
        </div>

        <button type="button" className="back-shoot-button" onClick={() => navigate(`/shoots/view/${id}`)}>
          Back
        </button>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="shoot-form-grid">
            ...
            <div className="shoot-form-group">
              ...
              <select id="creator" name="creator" value={formData.creator} onChange={handleChange}>

...
function ShootCalendar() {
  ...
  useEffect(() => {
    const loadShoots = async () => {
      try {
        ...
        setShoots(data.shoots || data.data || []);
      } catch (error) {
        console.error("Get shoots error:", error);
        toast.error(error.response?.data?.message || "Failed to load shoots");
      } finally {
        setLoading(false);
      }
    };
  ...
  const formatShootDateTime = (shootDate, time) => {
    if (!shootDate || !time) {
      return null;
    }
  ...
  const calendarEvents = shoots
    .map((shoot) => {
      const start = formatShootDateTime(shoot.shootDate, shoot.startTime);
      const end = formatShootDateTime(shoot.shootDate, shoot.endTime);
      ...
      return {
        id: shoot._id,
        title: shoot.creator?.name || "Shoot",
        start,
        end,
        resource: shoot,
      };
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <button type="button" onClick={async () => {
              try {
                await deleteShoot(selectedShoot._id);
            ...
            <button type="button" onClick={closeToast} style={{ border: "1px solid #d1d5db", borderRadius: "6px", padding: "7px 12px", background: "#fff", color: "#374151", cursor: "pointer", fontWeight: "600" }}>
              Cancel
            </button>
          </div>
        </div>
  ...
  return (
    <div className="shoots-page">
      ...
      {selectedShoot && (
        <div className="shoot-modal-overlay" onClick={() => setSelectedShoot(null)}>
          <div className="shoot-modal" onClick={(e) => e.stopPropagation()}>
            <div className="shoot-modal-header">
              ...
              <button type="button" className="shoot-modal-close" onClick={() => setSelectedShoot(null)}>
                <X size={20} />
              </button>
            </div>
